...
> 当前公开稳定版：**v0.2.0**。当前开发候选：**v0.3.0-dev.1**。
> v0.3.0 必须经 Developer ID 签名、Apple 公证和 Gatekeeper 验证后才可发布。

---

## v0.3.0（待发布）

> 状态：产品与体验候选已通过第 16 轮独立用户验收；正式发布等待 Developer ID
> Application 证书、Apple 公证 profile 和干净机器安装验收。当前开发 DMG 不得公开分发。
...
- 当前质量基线：335 项测试通过、覆盖率 84%，分层回归 6/6。
- 正式构建强制 Developer ID、hardened runtime、公证、staple 和 Gatekeeper 门禁；开发签名产物不能降级进入发布渠道。
