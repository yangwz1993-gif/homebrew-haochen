...
> 本次发布：**v0.3.2**；上一公开稳定版：**v0.3.1**。
> 所有者已明确选择 v0.3.2 沿用 legacy Homebrew 分发；它未经 Apple 公证。
...
- macOS 结果气泡使用 Quartz 系统光标与窗口几何可靠判定跨应用悬停；连续结果各自获得独立完整的 20 秒预算。
- 当前质量基线：367 项测试通过、覆盖率 83%，分层回归 6/6；独立高审美用户黑盒验收第 11 轮 PASS，P0/P1/P2 均为 0。
