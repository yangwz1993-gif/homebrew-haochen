...
> v0.2 安全修订：正式版仅允许 Developer ID + Apple 公证；本地自签方案已废止，不得用于构建或分发。
>
...
2. **稳定签名 → 归因正确 + 授权持久**：用 **Apple Developer ID** 稳定签名 → DR 固定 → 权限归到 app 本身 → 授权可跨升级保持。
...
### 1. 先保证**稳定签名**（前提，最重要）
- 正式版使用 Apple Developer ID，确保 app 有唯一稳定的 DR。
- **不要分发 adhoc**（授权不持久 + 责任进程错）；ad-hoc 仅限显式 development 构建。
...
1. **Developer ID 签名必须是 release 打包默认**；缺少签名或公证凭据时 release 构建必须失败。
