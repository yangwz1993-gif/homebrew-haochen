...
bash -c "HAOCHEN_MOCK=1 MOCK_TICK_MS=30 QT_QPA_PLATFORM=offscreen HAOCHEN_HOME=/tmp/haochen-p6-test HAOCHEN_SKIP_ONBOARDING=1 HAOCHEN_AUTO_RESTART=1 '$VENV' '$APP/verification/p4_integration.py' 2>/dev/null | grep -q '28/28'"
check "L3 P4 集成 (25)"

if [ "$REAL" = "1" ]; then
  run_name "L3.5 真引擎全链路 (16)"
  bash -c "HAOCHEN_HOME=/tmp/haochen-p6-real HAOCHEN_SKIP_ONBOARDING=1 QT_QPA_PLATFORM=offscreen '$VENV' '$APP/verification/p4_real_chain.py' 2>/dev/null | grep -q '16/16'"
