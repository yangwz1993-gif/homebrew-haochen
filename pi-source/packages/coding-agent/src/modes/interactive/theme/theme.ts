...
// Embedded fallback for Bun compiled binaries, where theme JSONs are not on disk.
...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		...
		if (fs.existsSync(darkPath) && fs.existsSync(lightPath)) {
			...
			// Bun compiled binary ships without theme files on disk; use the bundled JSON.
			BUILTIN_THEMES = { dark: darkThemeJson, light: lightThemeJson };
		}
	}
